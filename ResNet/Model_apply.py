...
from tensorflow.keras.applications import ResNet50
from Denoise import wavelet, compare_plot, normalize,median_filter, butterworth,relength
import config
...
#model = Model(inputs = model.input, outputs = x)
    features,prediction= model(data)
    print(prediction)
    print(features)
    return features
...
if __name__ == '__main__':
    ...
    leads = leads_transfer(leads[1],shape=(1,50,180,1))
    features = res_feature(leads)