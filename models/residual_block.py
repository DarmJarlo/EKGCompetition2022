...
class Own_BottleNeck(tf.keras.layers.Layer):
    def __init__(self, filter_num, stride=1):
        ...
        self.conv1 = tf.keras.layers.Conv2D(filters=filter_num,
                                            ...
                                            padding='same')
        self.bn1 = tf.keras.layers.BatchNormalization()
        '''here i have tried the strides from 1 to 2 and the filters from num to num*2'''
        self.conv2 = tf.keras.layers.Conv2D(filters=filter_num,
                                            kernel_size=(3, 3),
                                            strides=stride,
                                            padding='same')
    ...
    def call(self, inputs, training=None, **kwargs):
        residual = self.downsample(inputs)
        #residual = inputs
        '''here i have added more bn and relu before the conv https://arxiv.org/abs/1603.05027'''
...
def OneD_bottleneck_layer(filter_num, blocks, stride=1):
    res_block = tf.keras.Sequential()
    #res_block.add(BottleNeck(filter_num, stride=stride))
    res_block.add(OneD_BottleNeck(filter_num, stride=stride))
    #res_block.add(Own_BottleNeck(filter_num, stride=stride))
